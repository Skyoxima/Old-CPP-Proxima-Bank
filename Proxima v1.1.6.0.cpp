...
class account
{
  private:
   ...
   double tempamt;
   char password[10],dis_number_name[80],del_number_name[80],disaccno[7],delaccno[7];
  ...
  public:
   ...
   void master_listin();                    //should have created an admin class , i mean look at the size of this class alone o.0
   void master_listout();
   void adminaccess();

   void check_del_accno();
   void del_info_in();
   void del_info_out();
   void admin_delete_intro();
   void admin_delete_num();
   void admin_delete_numname();
   void admin_delete_outro();
   void is_amount_zero();
};
...
void account::check_del_accno()
{
   ifstream del_in;
   del_in.open("Admdelno.txt");   //ios out to create the file admdelno.txt for a fresh new start otherwise the program hangs
   while(!del_in.eof())
   {
     del_in>>disaccno;
...
void account::is_amount_zero()
{
   strcpy(accamount,accno);
    strcat(accamount,"amt.txt");

   ifstream delamt;
   delamt.open(accamount);
   delamt>>tempamt;
   delamt.close();

   if(tempamt!=0)
   {
      cout<<"\n\nSince account's current balance is non-zero (or negative) , \nthis account cannot be disabled or deleted.\n";
      cout<<"Your account's deletion request is still recorded and will not be withdrawn.\n";
      cout<<"To have your account disabled , the balance amount should be 0.00 INR.\n";
   }
   else
   {
     cout<<"\nThis account had been requested to be disabled and it was confirmed"; dots();
     cout<<"No more transactions can be carried out on this account.\n\n";
     cout<<"Session being terminated"; dots();  dots();
     exit(0);
   }
}

void account::del_info_in()
{
   ifstream del_in;
   del_in.open("Admindel.txt");  // to save the program from hanging.
   while(!del_in.eof())
   {
     del_in.getline(dis_number_name,80);           //NOT WORKING...FIX ASAP!
     admin_delete_numname();
   }
   del_in.close();

   del_in.open("Admdelno.txt");
   while(del_in.eof())
   {
      del_in>>disaccno;
      admin_delete_num();
...
void account::admin_delete_numname()
{
  ifstream adminlist;
  ofstream newlist;
  adminlist.open("Admin.txt");
  newlist.open("temp1.txt");
  while(!adminlist.eof())
  {
     adminlist.getline(del_number_name,80);
     if(strcmp(dis_number_name,del_number_name)!=0)
     {
       newlist<<del_number_name<<endl;
     }
  }
  adminlist.close();
  newlist.close();
...
void account::admin_delete_num()
{
   strcpy(accno,disaccno);
   copy_cat();
   ...
   {
	textcolor(12);
	cout<<"All the records of account number "; cprintf("%s",accno);
	cout<<" have been successfully deleted.\n";
	textcolor(15);
   }
}

void account::del_info_out()
{
   ifstream accnos;
   ofstream del_out;
   accnos.open("Accounts.txt");
   del_out.open("temp.txt");
   while(!accnos.eof())
   {
      accnos>>delaccno;
      if(strcmp(disaccno,delaccno)!=0)
      {
	del_out<<delaccno<<endl;
      }
   }
   accnos.close();
   del_out.close();
...
void account::admin_delete_intro()
{
    clrscr();
...
void account::admin_delete_outro()
{
   ofstream adminfiles;
   adminfiles.open("admdelno.txt",ios::trunc);
   adminfiles.close();

   adminfiles.open("Admindel.txt",ios::trunc);
   adminfiles.close();
}
...
void loan::no_sec_loan()
{
  ...
  loan_pen_check.open(loan_amt_file,ios::out);     //check if opening  amount file works
...
class mainpage : public loan        //might turn it protected loan
{
  ...
  int criteria;
  char accholder[10];
  long int timecarrier;

  public:
  mainpage()
  {
     ofstream admdelno;
     admdelno.open("Admdelno.txt",ios::in);
     admdelno.close();

     admdelno.open("Admindel.txt",ios::in);
     admdelno.close();
  }
  void mainlist();
  void misc_options();
...
void mainpage::request_deletion()
{
     ...
     textcolor(12);
     cout<<"The account of number "; cprintf("%s",accno); cout<<" will be deleted permanently , \nplease confirm request submission : ";
     cin>>criteria; textcolor(15);

     if(criteria==1)
     {
       cout<<"\nConfirmation submitted"; dots();
       cout<<"Effective immediately this account is marked disabled ;\nno further transactions will be entertained for this account.\n";
       cout<<"Proceeding"; dots();
       ...
       delfile.open("Admdelno.txt",ios::app);
       delfile<<accno<<"\n";
       delfile.close();
     }
     else if(criteria==0)
