...
class account
{
  private:
   ...
   double tempamt;
   char password[10],number_name[80],disaccno[7];
...
{
  ifstream delin,del_accno_in;
  delin.open("Admindel.txt");
  del_accno_in.open("Admdelno.txt");
  delin.getline(number_name,80);
  del_accno_in>>disaccno;
  delin.close();
  del_accno_in.close();
}

void account::is_amount_zero()
{                               	//could not make it friend of bank...in_amount is not reusable
   strcpy(accamount,disaccno);
    strcat(accamount,"amt.txt");

   ifstream checkamt;
   checkamt.open(accamount);
   checkamt>>tempamt;
   checkamt.close();

   if(tempamt!=0)
   {
      cout<<"Your account balance is non-zero\nAccording to our policy we cannot disable or delete your account\ntill its balance is zero [0] INR.";
      dots();
      cout<<"First empty your account to have it closed/deleted.\n";
   }
   else
   {
      cout<<"Since the current balance of this account is zero ,\nthis account is disabled till it is deleted by our admin.\n";
      cout<<"Terminating this session"; dots(); dots();
      exit(0);
   }
}

void account::check_disable()
{
  if(strcmp(disaccno,accno)==0)
  {
     cout<<"This account was requested and confirmed to be deleted hence it is disabled\nNo transactions will be carried out with this\naccount number anymore!\n";
     cout<<"Terminating this session"; dots(); dots();
     exit(0);
  }
}
...
{
   cout<<"Would you like to carry out another function?\n(Press 1 for YES or 0 for NO) : ";
   cin>>criteria;
   if(criteria==1)
   {
      cout<<"Proceeding"; dots();
